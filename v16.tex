...
\sS{Satz Komplexe Exponentialreihe konvergiert absolut}
...
\item{\alg{|exp(ix)|^2=exp(ix)·\ol{exp(ix)}\underset{4)}{=}exp(ix)·exp(\ol{ix})=exp(ix)·exp(-ix)&\underset{2)}{=}exp(ix-ix)=exp(0)\underset{1)}{=}1\\
&\overset{\sqrt{}}{\Rarr}\ |exp(ix)|=1}}
\end{enumerate}
