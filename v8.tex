...
\subsection*{Konvergenzsätze (Wdh.)}
\begin{itemize}
...
\section{Satz Verdichtungslemma von Candy}
Sei $(a_n)$ monoton fallende Nullfolge.\\
...
\bsp
    $a_k = \frac{1}{k}$ \phantom{XXX} $(k \geq 1)$\\
...
\underline{Beweis:}
Sei $b_n = \displaystyle\sum_{k=2^n}^{2^{n+1}-1} a_k$\\
...
$\equ 1-a<0 \equ a>1 \hfill \square$\\
\\
Beziehung:
...
$\to $ streng monotom wachsende Teilfolge gefunden. $\square$
}
\end{itemize}
\section{Satz Bolzano-Weierstraß}
Jede beschränkte Folge reeller Zahlen hat eine konvergernte Teilfolge.\\
\sss{Beweis}
Es gibt ein monotone Teilfolge (Lemma 4.14)\\
Diese ist beschränkt \Rarr konvergent. $\hfill\square$\\
\section{Definition}
Eine Folge reeller Zahlen $(a_n)_{n \geq 0}$ heißt Cauchyfolge wenn gilt:\\
Für jedes $\epsilon > 0$ gibt es ein $N \in \N$ sodass für $m, n \geq N$ gilt: $|a_n - a_m| < \epsilon$
\section{Satz}
Eine Folge reeller Zahlen $(a_n)$ konvergiert genau dann, wenn sie eine Cauchyfolge ist.\\
\sss{Beweis:}
$\Rarr$ Sei $a_n \to a$ für $n \to \infty$\\
Gegeben sei $\epsilon > 0$. Es gilt $N \in \N$ so dass $|a_n - a| < \frac{\epsilon}{2}$ für $n \geq N$\\
Für $n, m \geq N$ gilt:\\
