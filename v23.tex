...
Sei $f:[a,b]→\R$ stetig, $F:[a,b]→\R$ Stammfunktion von $f$, dann $$\int_a^{b} f(x)dx=F(b)-F(a)=:F(x) \vert\ary{b\\a}$$
\bew
...
(Unbestimmte Integrale) $\underset{z.B.}{\Rarr}\ \Int_a^b e^xdx=e^x\vert\ary{b\\a}=e^b-e^a$ etc.
...
$$(F·\phi)'(x)=F'(\phi(x))·\phi'(x)=f(\phi(x))·\phi'(x)\ \Rarr \int_{\phi(x)}^{\phi(b)}f(x)dx=F(x)\vert\ary{\phi(b)\\\phi(a)}
=F(\phi(b)-F(\phi'(a))$$
...
$f(x)=e^x=\frac{1}{2}e^2\vert\ary{1\\0}=\frac{e-1}{2}$
$F(\phi(X)) \mid_a^b = \int_a^b (F(\phi \circ F)'(x)) dx = \int_a^b f(\phi (x)) \cdot \phi'(x)dx$
}
\bsp
\enum{
