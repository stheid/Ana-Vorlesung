...
\bem % TODO Schönheitskorrekturen
$f$ ist $\left{}injektiv\\ surjektiv\\ bijekti \right}$v genau dann wenn für jedes $b \in B$ $\left{ \begin{array}{2} \text{höchstens} \text{mindestens} \text{genau} \end{array} \right}$ ein $a \in A mit $f(a) = b$\\
...
\item[g]{ist surjekt}v, denn für jedes $b \in \R_{geq 0}$ gilt f(\sqrt{b}) = b also igibt es $ \in \Ra
...
log = exp()^{-1} \R_{\geq } \to \Ra

\end{iteize}

\underline{\section*{Bild und Urbild}}
...
\item{Für eine Teilmenge $Y\subseteqB$ ist $f^{-1:=\{a\in A|f(a)\in Y\}\subseteq A$ das Urbild von $Y$ unter $f$
...
\underline{\section*{Funktionen}}
\sS{Definition}
Sei $D\subseteq\R$ Teilmenge. Eine reele Funktion\\
auf D ist eine Abbildung $f:D→\R$\\
...
seien $a, b \in \R$ $[a, b] = \{x \in \R| a \leq x \leq b\}$ (abgeschlossen)\\
...
Sei $D = {x \in \R | g(x) \geq 0}\ledsto \dfrac{f}{g} : D \to , Rx \mapsto \frac{f(x)}{g(x)}$
Solche Funktionen heién rationale Funktionen.
}
\bsp
$f:\R\bs\{0,1\}→\R, \ f(x)=\dfrac{x^7+5x^2}{x(x-1)}
\end{itemize}
\sS{Definition}
Seien $f: D \to R, g: D \to \R$ Funktionen sodass $f(C) \sebseteq D$
Eine Kompsition von f und g ist 
